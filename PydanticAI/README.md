...
下表中「状态」表示当前仓库覆盖情况，「计划」表示后续可补充的学习方向。
...
| **业务集成** | Slack/Modal 潜在客户筛选等 | 📋 计划 | 可做：消息推送、定时任务、外部 API 封装 |
| **UI 示例** | AG-UI、对话组件 | 📋 计划 | 可做：Gradio 定制、前端组件与样式规范 |
...
3. **学习计划**：上表「计划」项可按优先级拆成小任务（如先做 SQL 生成、再做 RAG），每完成一块可在本 README 或对应目录下加链接与简短说明。
